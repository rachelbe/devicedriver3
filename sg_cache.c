...
{
	cache = malloc((32) * sizeof(blockstruct));  //create cache array
	

	for(int i=0;i<32;i++)  //set occupied to 0
	{
		cache[i].occupied =0;
...
{
	int countlen =0;
	for (int i =0; i<32; i++)  //loops through cache
	{
...
int addin(uint64_t  node,uint64_t  block, char *data)
{
	...
	for(int k=0;k<32;k++)  //find the highest time
	{
...
{
	for (int i=0;i<32;i++) //loop through cache
	{
...
int putSGDataBlock( SG_Node_ID nde, SG_Block_ID blk, char *block ) 
{
	...
	if (emptcount == 32)  //if the cache is full
	{
