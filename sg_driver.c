...
structmake filearr [filearr_size];
...
// FILL IN THE REST OF THE CODE
    initSGCache(32); //can initSGCache which creates the cache
...
//if statment for when you have to write to the second half (need to read in first)
    ...
    if (filearr[search].file_pointer%1024 != 0)  //if writing to second half of the block
    {
...
//set file pointer to file size
    ...
    else //fp is less than file size and its divisible by 1024
    {
    ...
    //get the block and node from the rigth spot in the file struct
        ...
        if (getit != NULL)
        {
            ...
            for (int i=0; i<1024;i++)
            {
                writebuff[i]=getit[i]; //copy data into writebuff
            }
